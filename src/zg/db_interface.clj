...
(defn add-new-word-into-dictionary
  ...
  ([word user-name dictionary-type]
   (let [datetime (format-date/format-current-date)]
     ...
     (try (insert-word-into-dictionary word
                                       nil
                                       nil
                                       user-name
                                       dictionary-type
                                       datetime)
          (catch Exception e (log/error e "insert word into dictionary")))))
  ...
  ([word description class use-it internal copyright source correct-forms incorrect-forms user-name dictionary-type]
   (let [datetime (format-date/format-current-date)]
     (log/info "storing" datetime word)
     (try (insert-word-into-glossary word
                                     description
                                     class
                                     use-it
                                     internal
