...
(defn render-search-field
  ...
  (form/form-to
    ...
    [:get (search-href url-prefix mode)]
    [:div {:class "input-group"}
     [:span {:class "input-group-addon"} "Search"]
     (form/text-field
           {:size "40", :class "form-control", :placeholder "Search for word"}
           "word"
           (str word))
...
(defn render-name-field
  ...
  (form/form-to
    ...
    [:div {:class "input-group"}
     ...
     (form/text-field
           {:size "10", :class "form-control", :placeholder "User name"}
           "user-name"
           (str user-name))
     [:div {:class "input-group-btn"}
...
(defn render-navigation-bar-section
  ...
  [:nav {:class "navbar navbar-inverse navbar-fixed-top", :role "navigation"} ; use
                                                                              ; navbar-default
                                                                              ; instead
                                                                              ; of
                                                                              ; navbar-inverse
...
(defn render-error-page
  ...
  (page/xhtml
    (render-html-header "" url-prefix nil) ; TODO change, it might not work
                                           ; correctly (title, mode)
    [:body
     [:div {:class "container"}
          (render-navigation-bar-section user-name url-prefix nil) ; TODO
                                                                   ; change, it
                                                                   ; might not
                                                                   ; work
                                                                   ; correctly
...
(defn render-front-page-for-whitelist
  ...
  [:div {:class "container-fluid"}
   ...
   [:br]
   (form/form-to
       [:post (str url-prefix "add-words-to-whitelist")]
...
(defn render-front-page-for-blacklist
  ...
  [:div {:class "container-fluid"}
   ...
   [:br]
   (form/form-to
       [:post (str url-prefix "add-word-to-blacklist")]
       [:table {:style "border-collapse: separate; border-spacing: 10px;"}
        ...
        [:tr [:td [:div {:class "_label _label-default"} "Class:"]]
             ...
             [:td (form/drop-down
                            "class"
                            ["N/A" "Noun" "Verb" "Adjective" "Adverb" "Pronoun" "Preposition" "Conjunction" "Determiner" "Exclamation"])]]
        ...
        ;     [:td "&nbsp;"]
        ;     [:td (form/drop-down "source" (map (fn [i] [(:source i)(:id i)])
        ;     sources) )]]
        [:tr [:td "&nbsp;"]
             [:td "&nbsp;"]
...
(defn render-front-page-for-atomic-typos
  ...
  [:div {:class "container-fluid"}
   ...
   [:div {:class "row"}
      ...
      [:div {:class "navbar-header"}
         [:a {:href (str url-prefix "deleted-words-in-atomic-typos"),
                  :class "navbar-brand"} "Deleted words"]
        ] ; ./navbar-header
      [:div {:style "width:80%"}
         (render-search-field word url-prefix mode)
        ]
   ...
   [:br]
   (form/form-to
       [:post (str url-prefix "add-word-to-atomic-typos")]
       [:table {:style "border-collapse: separate; border-spacing: 10px;"}
...
(defn render-front-page-for-glossary
  ...
  [:div {:class "container-fluid"}
   ...
   [:br]
   (form/form-to
       [:post (str url-prefix "add-word-to-glossary")]
       [:table {:style "border-collapse: separate; border-spacing: 10px;"}
        [:tr [:td [:div {:class "_label _label-primary"} "New word:"]]
             [:td "&nbsp;"]
             [:td (form/text-field {:size "30"} "new-word")]]
        [:tr [:td [:div {:class "_label _label-default"} "Description:"]]
             [:td "&nbsp;"]
             [:td (form/text-field {:size "50"} "description")]]
        [:tr [:td [:div {:class "_label _label-default"} "Class:"]]
             [:td "&nbsp;"]
        ...
        [:tr [:td [:div {:class "_label _label-default"} "Correct forms"]]
             [:td "&nbsp;"]
             [:td (form/text-field {:size "30"} "correct-forms")]]
        [:tr [:td [:div {:class "_label _label-default"} "Incorrect forms"]]
             [:td "&nbsp;"]
             [:td (form/text-field {:size "30"} "incorrect-forms")]]
        [:tr [:td "&nbsp;"]
             [:td "&nbsp;"]
             [:td (form/submit-button {:class "btn btn-primary"} "Add new word")]]]
       [:br])
  ...
  (page/xhtml
    ...
    [:body
     [:div {:class "container"}
          ...
          (case mode
                :whitelist
                  (render-front-page-for-whitelist word url-prefix emender-page mode)
                :blacklist
                  (render-front-page-for-blacklist word url-prefix emender-page mode)
                :atomic-typos
                  (render-front-page-for-atomic-typos word url-prefix emender-page mode)
                :glossary (render-front-page-for-glossary word
                                                          url-prefix
                                                          emender-page
                                                          mode
                                                          sources-map))

          (if message [:div {:class "label label-warning"} message])
          (if search-results
                [:table {:class "table table-stripped table-hover", :style "width:auto"}
                 [:tr [:th "Word"]
                        (if (= mode :atomic-typos) [:th "Correct form(s)"])
                        (if (not= mode :glossary) [:th "Added at"])
                        (if (not= mode :glossary) [:th "Added by"])
                        (if (or (= mode :blacklist) (= mode :glossary)) [:th "Description"])
                        [:th "Class"]
                        [:th "Use it"]
                        [:th "Internal"]
                        [:th "Copyright"]
                        [:th "Verified"]
                        (if (= mode :glossary) [:th "Incorrect/preferred forms"])
                        [:th "Source"]
                        (if (= mode :glossary) [:th "See also"])
                        [:th "Status"]
                        [:th "Operation"]]
                 (for [search-result search-results]
                         (let [deleted (= (:deleted search-result) 1)
                           ...
                           [:tr [:td word]
                            (if (= mode :atomic-typos) [:td (:correct search-result)])
                            (if (not= mode :glossary) [:td (:datetime search-result)])
                            ...
                            (if (= mode :glossary) [:td (:see_also search-result)])
                            [:td {:style (if deleted "color:red" "color:green")} (if deleted "deleted" "active")]
                            [:td (if deleted
                                                       [:a {:href (str "?undelete=" word), :class "btn btn-default"} "undelete"]
